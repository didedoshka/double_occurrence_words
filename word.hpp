...
inline int recursive(const word_t& word, int i, std::vector<int>& v) {
    ...
    for (int j = 0; i + j <= std::ssize(word) - 2; ++j) {
        v.push_back(i + j);
        result = std::min(result, recursive(word, i + j + 2, v));
        v.pop_back();
    }
...
// Expects word in ascending order
inline int assembly_number(const word_t& word) {
    std::vector<int> v;
...
inline void draw_as_graph(const word_t& word, std::string graph_name, bool calculate_assembly_number = false,
                          bool using_ports = true) {
    std::ofstream dot("data/" + graph_name + ".dot");
    dot << "digraph {\nfontname=\"Helvetica\"\nnode [fontname=\"Helvetica\"]\nnode [shape=diamond]\nlabel=\""
        << get_data(word, calculate_assembly_number || (len(word) <= 7), true) << "\"\n";
